...
const emptyCartDb = async (cartId) => {
  return await pool.query("delete from cart_id where cart_id = $1", [cartId]);
};

const getCartByTelegramIdDb = async (telegramId) => {
  ...
  if (userRows.length === 0) {
    throw new Error("Telegram ID'ye ait kullanıcı bulunamadı.");
  }
...
const getCartByUserIdDb = async (userId) => {
  const cart = await pool.query(
    `SELECT products.*, cart_item.quantity, round((products.price * cart_item.quantity)::numeric, 2) as subtotal
    FROM users
    JOIN cart ON users.user_id = cart.user_id
